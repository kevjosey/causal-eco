...
create_strata <- function(aggregate_data, x, phat.vals,
                          dual = c("all","high","low"),
...
lapply(1:nrow(scenarios), function(i, ...) {
  ...
  new_data <- create_strata(aggregate_data = aggregate_data, x = x, phat.vals = phat.vals, dual = scenario$dual, race = scenario$race)
  save(new_data, file = paste0(dir_data, "qd2/", scenario$dual, "_", scenario$race, ".RData"))
