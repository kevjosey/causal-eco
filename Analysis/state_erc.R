...
# run it!
mclapply(seq(1, nrow(scenarios), by = 1), function(i, ...) {
  ...
  a.vals <- seq(ceiling(max(5, min(x$pm25))), 
                floor(min(15, max(x$pm25))), length.out = 101)
  ...
  save(new_data, file = paste0(dir_out, scenario$state, "_", scenario$dual, ".RData"))
