...
mod_sd<-mod_sd_rm
feature_names<-feature_names_rm
GPS_mod<-GPS_mod_rm
...
f <- list.files("/nfs/home/P/prd789/shared_space/ci3_analysis/pdez_measurementerror/National_Causal-master/FST_data_rm/",
                pattern = "\\.fst",
                full.names = TRUE)
mc_cores=4
...
par.match.noerrer<-function(a_i=a_i,data.list,GPS_mod=GPS_mod,delta_n=delta_n,scale=scale){
  matching_noerror_level <- data.table(Reduce(rbind,lapply(1:length(data.list),function(i,a_i=a_i,GPS_mod=GPS_mod,delta_n=delta_n,scale=scale){
...
saveRDS(match.noerrer, paste0(dir_out, "Matching_Output_rm/all/", process, ".rds"))
