gam_om <- function(x, w, z, se.fit = TRUE, boot = FALSE, 
  ...
  if (se.fit) {
    ...
    invbread <- meat <- matrix(0, ncol = ncol(wx.mat), nrow = ncol(wx.mat))
    
    # sandwich estimator mess
    for (i in 1:length(wx$ybar)) {
      
      invbread <- invbread - wx$n[i]*family$mu.eta(family$linkfun(muhat[i]))*tcrossprod(wx.mat[i,])
      meat <- meat + tcrossprod(wx$n[i]*(wx$ybar[i] - muhat[i])*wx.mat[i,])
  ...
  out.vals <- sapply(a.vals, function(a.tmp, ...) {
    ...
    if (se.fit) {
      sig2 <- c(t(delta) %*% wx.tmp %*% Sigma %*% t(wx.tmp) %*% delta)/(sum(wx$n)^2)
      omega2 <- c(t(cut) %*% (delta*wx.tmp) %*% Sigma %*% c(t(delta*wx.tmp) %*% cut))
