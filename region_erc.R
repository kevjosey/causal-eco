...
# run it!
mclapply(seq(2,nrow(scenarios), by = 2), function(i, ...) {
  ...
  full_data <- erc_implement(x = x, w = w, z = z, a.vals = a.vals,
                      se.fit = TRUE, boot = FALSE, region = scenario$region)
  
  # fit model on bootstrap
  boot_data <- replicate(nboot, bootable(w = w, x = x, z = z, a.vals = a.vals, region = scenario$region), simplify = FALSE)
