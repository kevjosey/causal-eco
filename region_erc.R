...
# run it!
mclapply(c(22, 29, 43, 44, 46, 47, 49), function(i, ...) {
  ...
  if (scenario$region == "all") {
    region0 <- c("MIDWEST", "NORTHEAST", "SOUTH", "WEST")
  } else {
    region0 <- region
  }
  ...
  sub_data <- subset(aggregate_data, race %in% race0 & dual %in% dual0 & region %in% region0)
  x0 <- data.table(zip = factor(sub_data$zip), year = factor(sub_data$year),
                   model.matrix(~ ., data = sub_data[,z])[,-1])[,lapply(.SD, min), by = c("zip", "year")]
  x0 <- data.table(setDF(x0)[,-which(colnames(x0) == "pm25")] %>% mutate_if(is.numeric, scale), pm25 = x0$pm25)
  x <- merge(x0, data.table(zip = factor(sub_data$zip), year = factor(sub_data$year),
                            m = sub_data$time_count)[,lapply(.SD, sum), by = c("zip", "year")], by = c("zip", "year"))
  
  # individual-level predictors
  w <- data.table(zip = factor(sub_data$zip), year = factor(sub_data$year),
                  y = sub_data$dead, n = sub_data$time_count)[,lapply(.SD, sum), by = c("zip", "year")]
  # w <- data.table(zip = factor(sub_data$zip), year = factor(sub_data$year),
  #                 dual = factor(sub_data$dual), race = factor(sub_data$race),
  #                 sex = factor(sub_data$sex), age_break = factor(sub_data$age_break),
  #                 y = sub_data$dead, n = sub_data$time_count)[,lapply(.SD, sum), by = c("zip", "year", "dual", "race", "sex", "age_break")]
  ...
  new_data <- list(est_data = full_data$est_data, excess_death = full_data$excess_death, 
                   # boot_erc = boot_erc, boot_ed = boot_ed, 
                   wx = full_data$wx)
  save(new_data, file = paste0(dir_out, scenario$dual, "_", scenario$race, "_", scenario$region, ".RData"))
