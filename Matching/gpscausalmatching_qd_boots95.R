...
q1<-quantile(prematch_data1$pm25_ensemble, 0.05)
q2<-quantile(prematch_data1$pm25_ensemble, 0.95)
trimed_index<-which(a.vals >=q1 & a.vals <=q2)

all_qd.list<-split(prematch_data1, list(prematch_data1$zip))

num_uniq_zip <- length(unique(prematch_data1$zip))
zip_sample<-sample(1:num_uniq_zip,floor(2*sqrt(num_uniq_zip)),replace=T) 
prematch_data<-data.frame(Reduce(rbind, all_qd.list[zip_sample]))
...
#Strata
#White female
prematch_data1<-prematchdata(white_female_qd, covariates_white_female_qd)
all_qd.list<-split(prematch_data1, list(prematch_data1$zip))
...
#White male
prematch_data1<-prematchdata(white_male_qd, covariates_white_male_qd)
all_qd.list<-split(prematch_data1, list(prematch_data1$zip))
...
#Black female
prematch_data1<-prematchdata(black_female_qd, covariates_black_female_qd)
all_qd.list<-split(prematch_data1, list(prematch_data1$zip))
...
#Black male
prematch_data1<-prematchdata(black_male_qd, covariates_black_male_qd)
all_qd.list<-split(prematch_data1, list(prematch_data1$zip))
...
#Hispanic female
prematch_data1 <-prematchdata(hispanic_female_qd, covariates_hispanic_female_qd)
all_qd.list<-split(prematch_data1, list(prematch_data1$zip))
...
#Hispanic male
prematch_data1<-prematchdata(hispanic_male_qd, covariates_hispanic_male_qd)
all_qd.list<-split(prematch_data1, list(prematch_data1$zip))
...
#Asian female
prematch_data1<-prematchdata(asian_female_qd, covariates_asian_female_qd)
all_qd.list<-split(prematch_data1, list(prematch_data1$zip))
...
#Asian male
prematch_data1<-prematchdata(asian_male_qd, covariates_asian_male_qd)
all_qd.list<-split(prematch_data1, list(prematch_data1$zip))
