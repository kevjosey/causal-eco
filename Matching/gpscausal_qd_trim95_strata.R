...
for(i in 1:length(files)){
  ...
  #ERC
  matchingqd_gam1 <-gam(dead~ s(pm25_ensemble,bs='cr',k=3) +
                                as.factor(sex)+as.factor(race)+as.factor(dual)+as.factor(entry_age_break)+as.factor(followup_year)+
                                offset(log(time_count))
                              , data=aggregate_data_qd2, family=poisson(link="log"), weights=counter)
  print("Evaluating ERC all")
  
  white_femaleqd_matching_gam1 <-gam(dead~ s(pm25_ensemble, bs='cr',k=3) +
                                             as.factor(dual)+as.factor(entry_age_break)+as.factor(followup_year)+
                                             offset(log(time_count))
                                           , data=match_pop_white_female_qd2,family=poisson(link="log"),
                                           weights=counter)
  ...
  white_maleqd_matching_gam1 <-gam(dead~ s(pm25_ensemble, bs='cr',k=3) +
                                           as.factor(dual)+as.factor(entry_age_break)+as.factor(followup_year)+
                                           offset(log(time_count))
                                         , data=match_pop_white_male_qd2,family=poisson(link="log"),
                                         weights=counter)
  ...
  black_femaleqd_matching_gam1 <-gam(dead~ s(pm25_ensemble, bs='cr', k=3) +
                                             as.factor(dual)+as.factor(entry_age_break)+as.factor(followup_year)+
                                             offset(log(time_count))
                                           , data=match_pop_black_female_qd2,family=poisson(link="log"), weights=counter)
  print("ERC Black female")
  
  black_maleqd_matching_gam1 <-gam(dead~ s(pm25_ensemble, bs='cr', k=3) +
                                           as.factor(dual)+as.factor(entry_age_break)+as.factor(followup_year)+
                                           offset(log(time_count))
                                         , data=match_pop_black_male_qd2,family=poisson(link="log"), weights=counter)
  print("Black male ERC ")
  hispanic_femaleqd_matching_gam1 <-gam(dead~ s(pm25_ensemble, bs='cr', k=3) +
                                                as.factor(dual)+as.factor(entry_age_break)+as.factor(followup_year)+
                                                offset(log(time_count))
                                              , data=match_pop_hispanic_female_qd2,family=poisson(link="log"),weights=counter)
  print("Hispanic female ERC")
  hispanic_maleqd_matching_gam1 <-gam(dead~ s(pm25_ensemble, bs='cr', k=3) +
                                              as.factor(dual)+as.factor(entry_age_break)+as.factor(followup_year)+
                                              offset(log(time_count))
                                            , data=match_pop_hispanic_male_qd2,family=poisson(link="log"), weights=counter)
  print("Hispanic male ERC")
  asian_femaleqd_matching_gam1 <-gam(dead~ s(pm25_ensemble, bs='cr',k=3) +
                                             as.factor(dual)+as.factor(entry_age_break)+as.factor(followup_year)+
                                             offset(log(time_count))
                                           , data=match_pop_asian_female_qd2,family=poisson(link="log"), weights=counter)
  
  print("Asia female ERC")
  asian_maleqd_matching_gam1 <-gam(dead~ s(pm25_ensemble, bs='cr', k=3) +
                                           as.factor(dual)+as.factor(entry_age_break)+as.factor(followup_year)+
                                           offset(log(time_count))
                                         , data=match_pop_asian_male_qd2,family=poisson(link="log"), weights=counter)
  print("Asian male ERC")
  tallqd[i,]<-test.data.qd(aggregate_data_qd2, matchingqd_gam1)
  print("All")
  ...
  print("Asian male")
  rm(matchingqd_gam1, white_femaleqd_matching_gam1,
     white_maleqd_matching_gam1, black_femaleqd_matching_gam1, 
     black_maleqd_matching_gam1, hispanic_femaleqd_matching_gam1, 
     hispanic_maleqd_matching_gam1, asian_femaleqd_matching_gam1,
     asian_maleqd_matching_gam1)
}
...
save(main_gnm, t_gnm,
     file='/nfs/nsaph_ci3/ci3_analysis/pdez_measurementerror/Output/Matching/boot_main_qd_assocations.RData' )
...
testallqd$pm25_ensemble<-seq(min(aggregate_data_qd2$pm25_ensemble), 
                    max(aggregate_data_qd2$pm25_ensemble), length.out = 50)

pall_qd<-ggplot(data=testallqd, mapping=aes(x=pm25_ensemble))+
  geom_line(aes(y=testallqd), col="red")+
  geom_ribbon(aes(ymin=(lower), ymax=(higher)), alpha=0.1)+
  labs(x="qd PM2.5", y="All HR")+theme_bw() + theme(legend.position="none")
...
test_white_female_qd$pm25_ensemble<-seq(min(match_pop_white_female_qd2$pm25_ensemble), max(match_pop_white_female_qd2$pm25_ensemble), length.out = 50)
...
test_white_male_qd$pm25_ensemble<-seq(min(match_pop_white_male_qd2$pm25_ensemble), max(match_pop_white_male_qd2$pm25_ensemble), length.out = 50)
...
test_black_female_qd$pm25_ensemble<-seq(min(match_pop_black_female_qd2$pm25_ensemble), max(match_pop_black_female_qd2$pm25_ensemble), length.out = 50)
...
test_black_male_qd$pm25_ensemble<-seq(min(match_pop_black_male_qd2$pm25_ensemble), max(match_pop_black_male_qd2$pm25_ensemble), length.out = 50)
...
test_hispanic_female_qd$pm25_ensemble<-seq(min(match_pop_hispanic_female_qd2$pm25_ensemble), max(match_pop_hispanic_female_qd2$pm25_ensemble), length.out = 50)
...
test_hispanic_male_qd$pm25_ensemble<-seq(min(match_pop_hispanic_male_qd2$pm25_ensemble), max(match_pop_hispanic_male_qd2$pm25_ensemble), length.out = 50)
...
test_asian_female_qd$pm25_ensemble<-seq(min(match_pop_asian_female_qd2$pm25_ensemble), max(match_pop_asian_female_qd2$pm25_ensemble), length.out = 50)
...
test_asian_male_qd$pm25_ensemble<-seq(min(match_pop_asian_male_qd2$pm25_ensemble), max(match_pop_asian_male_qd2$pm25_ensemble), length.out = 50)

main_gnm[9]-1.96*sd(t_gnm$asian_male)*sqrt(2*sqrt(num_uniq_zip))/sqrt(num_uniq_zip)
main_gnm[9]+1.96*sd(t_gnm$asian_male)*sqrt(2*sqrt(num_uniq_zip))/sqrt(num_uniq_zip)

save(test_asian_female_qd, test_asian_male_qd, test_black_female_qd, test_black_male_qd,
     test_hispanic_female_qd, test_hispanic_male_qd, test_white_female_qd,
     test_white_male_qd, t_gnm,testallqd,
     file='/nfs/nsaph_ci3/ci3_analysis/pdez_measurementerror/Output/Matching/boot_main_qd.RData' )

load('/nfs/nsaph_ci3/ci3_analysis/pdez_measurementerror/Output/Matching/boot_main_qd.RData')
pall_qd<-ggplot(data=testallqd, mapping=aes(x=pm25_ensemble))+
  geom_line(aes(y=testallqd), col="red")+
  geom_ribbon(aes(ymin=(lower), ymax=(higher)), alpha=0.1)+
  labs(x="qd PM2.5", y="All HR")+theme_bw() + theme(legend.position="none")

p_white_female_qd<-ggplot(data=test_white_female_qd, mapping=aes(x=pm25_ensemble))+
  geom_line(aes(y=test_white_female_qd), col="red")+
  geom_ribbon(aes(ymin=(lower), ymax=(higher)), alpha=0.1)+
  labs(x="PM2.5", y="White female HR")+theme_bw() + theme(legend.position="none")

p_white_male_qd<-ggplot(data=test_white_male_qd, mapping=aes(x=pm25_ensemble))+
  geom_line(aes(y=test_white_male_qd), col="red")+
  geom_ribbon(aes(ymin=(lower), ymax=(higher)), alpha=0.1)+
  labs(x="PM2.5", y="White male HR")+theme_bw() + theme(legend.position="none")

p_black_female_qd<-ggplot(data=test_black_female_qd, mapping=aes(x=pm25_ensemble))+
  geom_line(aes(y=test_black_female_qd), col="red")+
  geom_ribbon(aes(ymin=(lower), ymax=(higher)), alpha=0.1)+
  labs(x="PM2.5", y="Black female HR")+theme_bw() + theme(legend.position="none")

p_black_male_qd<-ggplot(data=test_black_male_qd, mapping=aes(x=pm25_ensemble))+
  geom_line(aes(y=test_black_male_qd), col="red")+
  geom_ribbon(aes(ymin=(lower), ymax=(higher)), alpha=0.1)+
  labs(x="PM2.5", y="Black male HR")+theme_bw() + theme(legend.position="none")

p_hispanic_female_qd<-ggplot(data=test_hispanic_female_qd, mapping=aes(x=pm25_ensemble))+
  geom_line(aes(y=test_hispanic_female_qd), col="red")+
  geom_ribbon(aes(ymin=(lower), ymax=(higher)), alpha=0.1)+
  labs(x="PM2.5", y="Hispanic female HR")+theme_bw() + theme(legend.position="none")

p_hispanic_male_qd<-ggplot(data=test_hispanic_male_qd, mapping=aes(x=pm25_ensemble))+
  geom_line(aes(y=test_hispanic_male_qd), col="red")+
  geom_ribbon(aes(ymin=(lower), ymax=(higher)), alpha=0.1)+
  labs(x="PM2.5", y="Hispanic male HR")+theme_bw() + theme(legend.position="none")

p_asian_female_qd<-ggplot(data=test_asian_female_qd, mapping=aes(x=pm25_ensemble))+
  geom_line(aes(y=test_asian_female_qd), col="red")+
  geom_ribbon(aes(ymin=(lower), ymax=(higher)), alpha=0.1)+
  labs(x="PM2.5", y="Asian female HR")+theme_bw() + theme(legend.position="none")

p_asian_male_qd<-ggplot(data=test_asian_male_qd, mapping=aes(x=pm25_ensemble))+
  geom_line(aes(y=test_asian_male_qd), col="red")+
  geom_ribbon(aes(ymin=(lower), ymax=(higher)), alpha=0.1)+
