...
feature_names <- GPS_mod_qd$feature_names
#rm(list= ls()[!(ls() %in% c("feature_names"))])
mc_cores = 20

covariates<-covariates_qd
rm(covariates_qd)
...
covariates_boots<-aggregate(aggregate_data_boots[,c(10:25)], by=list(aggregate_data_boots$zip,
                                                                     aggregate_data_boots$year), FUN=min)
...
GPS_mod <-xgboost(data = data.matrix(covariates_boots[,feature_names]), 
                  label = covariates_boots$pm25_ensemble,nrounds=50)
...
Sys.time()
saveRDS(matching2, 
        paste0("/nfs/home/P/prd789/shared_space/ci3_analysis/pdez_measurementerror/National_Causal-master/Bootstrap/matchingqd/all/Matching_boot_", boots_id, ".rds") )
