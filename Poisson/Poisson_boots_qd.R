...
for (boots_id in 1:500){
  ...
  gnm_raw<-gnm(dead~  pm25_ensemble + 
               ...
               +offset(log(time_count))+(as.factor(dual)+as.factor(race) + as.factor(sex) +as.factor(entry_age_break)+as.factor(followup_year)), data=aggregate_data_boots,family=poisson(link="log"))
