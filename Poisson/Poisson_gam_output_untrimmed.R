...
#Main
#Main
load(paste0(dir_out_main, "Main_gam_rm_all.RData"))
load(paste0(dir_out, "gam1_boots_rm_all.RData"))
...
#Whitefemale
load(paste0(dir_out_main, "Main_gam_rm_white_female.RData"))
...
#main_white_female_rm$lower<- (rowQuantiles(as.matrix(test), prob=0.025))
#main_white_female_rm$higher<-(rowQuantiles(as.matrix(test), prob=0.975))
...
p_white_female_rm<-ggplot(main_white_female_rm, aes(x=a.vals)) +
  ...
  theme(panel.grid=element_blank(), legend.position="none")+ylim(0.5, 1.5)+
  coord_cartesian(xlim=range)+
  theme_cowplot()
...
#White Male
#White male
load(paste0(dir_out_main, "Main_gam_rm_white_male.RData"))
...
#main_white_male_rm$lower<- (rowQuantiles(as.matrix(test), prob=0.025))
#main_white_male_rm$higher<-(rowQuantiles(as.matrix(test), prob=0.975))
std<-apply((test), 1, sd, na.rm=TRUE)
...
load(paste0(dir_out_main, "Main_gam_rm_black_female.RData"))
load(paste0(dir_out,"gam1_boots_rm_all.RData"))
...
#Black male
load(paste0(dir_out_main, "Main_gam_rm_black_male.RData"))
...
#Hispanic female
dir_data = '/nfs/nsaph_ci3/ci3_analysis/pdez_measurementerror/Data/'
load(paste0(dir_out_main, "Main_gam_rm_hispanic_female.RData"))
...
#Hispanic male
load(paste0(dir_out_main, "Main_gam_rm_hispanic_male.RData"))
...
load(paste0(dir_out_main, "Main_gam_rm_asian_female.RData"))
...
p_asian_female_rm1<-ggdraw(aligned_plots[[1]])+
  draw_plot(aligned_plots[[2]])
#Asian male
load(paste0(dir_out_main, "Main_gam_rm_asian_male.RData"))
...
require(cowplot)
plot_grid(p_white_female_rm1 + ylim(0.5, 1.5), p_white_male_rm1+ ylim(0.5, 1.5), 
          p_black_female_rm1+ ylim(0.5, 1.5), p_black_male_rm+ ylim(0.5, 1.5), 
          p_hispanic_female_rm+ ylim(0.5, 1.5), p_hispanic_male_rm+ ylim(0.5, 1.5), 
          p_asian_female_rm+ ylim(0.5, 1.5), p_asian_male_rm+ ylim(0.5, 1.5), ncol=2, 
          labels="AUTO")
